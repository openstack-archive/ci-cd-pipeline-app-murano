...
def cleaup_up_tenant():
    ...
    for env in murano.environments.list():
        try:
            ...
            try:
                LOG.warning("Trying abandon env:{}".format(env))
                murano.environments.delete(env, abandon=True)
            except Exception as e:
                LOG.warning("Unable abandon env:{}".format(env))
                pass
            LOG.exception(e)
            pass
...
if __name__ == '__main__':
    ...
    #
    # murano = get_murano_client()
    # environment = murano.environments.get('400e7b1f7f65475b9e3b75e9d6f89f13')
    # service = 'jenkins'
