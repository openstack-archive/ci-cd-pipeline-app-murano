...
where:
  ...
  build packages options:
    -B  flag to build packages (default is: false) if true - script will build new packages
...
while getopts ':hUBs:d:p:e:' option; do
  case "$option" in
    h) echo "$help_string"
       exit
       ;;
...
if $build_packages || $upload ; then
    ...
    echo "WARNING: Packages were not built or uploaded. Use options -B or -U for it."
fi
