...
if $upload || $upload_without_build ; then
    ...
    # murano during rewriting packages rewrite all dependencies,
    # so we need to upload in reversed order and re-write them step by step
    reverse_packages=`for f in $default_packages ; do echo $f ; done | tac`
    # upload files in correct order
    for d in $reverse_packages; do
        filename="$destination_dir/io.murano.opaas.$d.zip"
