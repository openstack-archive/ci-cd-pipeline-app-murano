...
# def check_path(env, path, inst_name=None):
...
def deployment_success_check(environment, srv='jenkins', ports=[]):
    """
    :param environment:
    :param ports:
    """
    ...
    ip = environment.services[0][srv]['instance']['floatingIpAddress']

    if ip:
        for port in ports:
            LOG.debug("Looking into: {} {}:{} ".format(srv, ip, port))
            check_port_access(ip, port)
    else:
        fail('Instance "{}" does not have floating IP'.format(srv))
...
if __name__ == '__main__':
    ...
    environment = murano.environments.get('29a2c16f6af94e08bf06a926ded640c7')
