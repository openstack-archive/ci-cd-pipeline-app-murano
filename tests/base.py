...
class MuranoTestsBase(testtools.TestCase, clients.ClientsBase):
    ...
    @classmethod
    def setUpClass(cls):
        super(MuranoTestsBase, cls).setUpClass()

        cls.flavor = os.environ.get('OS_FLAVOR', 'm1.medium')
        cls.image = os.environ.get('OS_IMAGE', 'ubuntu-14.04-m-agent.qcow2')
        cls.keyname = os.environ.get('OS_KEYNAME', 'akuznetsova')
        cls.availability_zone = os.environ.get('OS_ZONE', 'nova')
        cls.deploy_timeout = (60 * 60) * 2
    ...
    def create_env(self):
        ...
        self.addCleanup(self.delete_env, environment.id)
    ...
    def get_env(self, environment_id):
        return self.murano.environments.get(environment_id)
    ...
    def check_port_access(self, ip, ports):
        for port in ports:
            result = 1
            start_time = time.time()
    ...
    def check_url_access(self, ip, path, port):
        ...
        self.fail(
            'Service path {0} is unavailable after 5 attempts, '
            'STATUS CODE={1}'.format(url, resp.status_code)
        )
