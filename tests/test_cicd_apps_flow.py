...
class MuranoCiCdFlowTest(base.MuranoTestsBase):
    ...
    def test_run_cicd_flow(self):
        ...
        self.make_commit(
            ...
            branch='master',
            key=self.pr_key,
            msg='Add jobs for petclinic app'
        )
        ...
        self.merge_commit(
            ...
            gerrit_host=gerrit_hostname,
            project='open-paas/project-config',
            commit_msg='Add jobs for petclinic app'
        )

        # Wait while new "petclinic-java-app-*" jobs are created

        self.wait_for(
            func=self.get_jenkins_jobs,
            expected='petclinic-java-app-test',
            debug_msg='Waiting while "petclinic-java-app-test" is created',
            fail_msg='Job "petclinic-java-app-test" wasn\'t created',
            timeout=600,
        ...
        # Check that 'petclinic-java-app-test' (it triggers on-commit) was run

        self.wait_for(
            self.get_last_build_number,
            expected=1,
            debug_msg='Waiting while "petclinic-java-app-test" '
                      'job is run and first build is completed',
            fail_msg='Job "petclinic-java-app-test" wasn\'t run on-commit',
            timeout=300,
            ip=fips['org.openstack.ci_cd_pipeline_murano_app.Jenkins'],
            user=ldap_user,
            password=ldap_password,
            job_name='petclinic-java-app-test',
            build_type='lastCompletedBuild'
        )

        # Check that 'petclinic-java-app-test' (it triggers on-commit) was
        # successfully finished

        self.wait_for(
            self.get_last_build_number,
            expected=1,
            debug_msg='Checking that first build of "petclinic-java-app-test" '
                      'job is successfully completed',
            fail_msg='Job "petclinic-java-app-test" has failed',
            timeout=600,
            ip=fips['org.openstack.ci_cd_pipeline_murano_app.Jenkins'],
            user=ldap_user,
            password=ldap_password,
